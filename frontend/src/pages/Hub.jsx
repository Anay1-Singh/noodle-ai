              ...
              {showDD && (
                <motion.div initial={{ opacity:0,y:-6,scale:0.96 }} animate={{ opacity:1,y:0,scale:1 }} exit={{ opacity:0,y:-6,scale:0.96 }} transition={{ type:'spring',damping:28,stiffness:380 }}
                  ...
                  <div style={{ padding: '5px 6px' }}>
                    ...
                    <button onClick={async () => { await fetch(`${import.meta.env.VITE_API_URL}/api/auth/logout`, { method: 'POST', credentials: 'include' }); localStorage.removeItem('user'); setShowDD(false); navigate('/'); }}
